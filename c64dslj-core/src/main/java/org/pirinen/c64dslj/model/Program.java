...
public class Program {
	...
	private Map<String, List<LabelOperandBase>> labelOperands; // for forward
																// branches
	...
	public void add(Label l) {
		if (labels.containsKey(l.getName())) {
			throw new RuntimeException("Label " + l.getName()
					+ " already exists");
		}
	...
	public byte[] getData() throws IOException {
		if (containsAbsoluteLabel() && startingAddress < 0) {
			throw new RuntimeException(
					"Instructions contain a label to absolute address, startAddress must be given");
		}
	...
	private static void fillWithZeroes(OutputStream out, int count)
			throws IOException {
